...
export class Backend implements backendInterface {
    ...
    async getBanner(arg0: string): Promise<Banner> {
        if (this.processError) {
            try {
                const result = await this.actor.getBanner(arg0);
                return result;
            } catch (e) {
                this.processError(e);
...
function from_candid_record_n2(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return {
        originalPrice: record_opt_to_undefined(from_candid_opt_n3(_uploadFile, _downloadFile, value.originalPrice)),
        name: value.name,
